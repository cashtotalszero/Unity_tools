...
public class JellyfishXMLgui : MonoBehaviour {
	...
	public void LoadPsml (ref int iSessionIndex) {
		...
		StringReader xmlDoc;
		TextAsset tPsml = (TextAsset)Resources.Load("psml",typeof(TextAsset));
		String sPsml, sPath, sTest = "\0";
		...
		// iOS require XML to be read as a TextAsset on first load
		#if UNITY_IPHONE

		// Path to the iPhone directory where you can write to
		sPath = Application.dataPath + "/../../Documents/psml";


		if(File.Exists(sPath)) {
			textAlex = "FILE EXISTS";
			sTest = File.ReadAllText(sPath);
			firstLoad = false;
		}
		else {
			textAlex = "FILE DOES NOT EXIST";
			firstLoad = true;
			tPsml = (TextAsset)Resources.Load("psml",typeof(TextAsset));
		}
		...
		if(firstLoad) {		 
			...
			xmlDoc = new StringReader (tPsml.text);
			validXml = xmlFormValid (tPsml.text);
		}
		else {
			xmlDoc = new StringReader (sTest);
			validXml = xmlFormValid (sTest);
		}
		#endif

		// Other platforms can read directly from file
		#if !UNITY_IPHONE
		sPsml = File.ReadAllText(Application.dataPath+"/psml.xml");
		xmlDoc = new StringReader (sPsml);
		validXml = xmlFormValid (sPsml);
		#endif
		...
		if (validXml) {
			...
			for (i=0; i<2; i++) {
				...
				if (firstLoad) {
					myReader = XmlReader.Create (new StringReader(tPsml.text));
					Debug.Log("First Load COMPLETE!!!!!!!!!!!!!!!!!!!!!! YES!!!!!!!!!!!");
				}
				else {
					myReader = XmlReader.Create (new StringReader(sTest));
				}
				...
				#if !UNITY_IPHONE
				myReader = XmlReader.Create (new StringReader(sPsml));
				#endif
		...
		//newSave (iSessionIndex, ref ocean); 

		#if UNITY_IPHONE
		sPath = Application.dataPath + "/../../Documents/psml.xml";
		if (File.Exists (sPath)) {
			Debug.Log ("OUTSIDE FUNCTION: SUCESSFULLY CREATED!!!!!!");
			string sTest2 = File.ReadAllText(sPath);
			Debug.Log(sTest2);
		} else {
			Debug.Log("Still no write...");
		}
		#endif
	...
	public void newSave(int iSessionIndex, ref List<Molecule> ocean) {
		...
		string sPath = Application.dataPath + "/psml.xml";
		XmlWriter xmlWriter = XmlWriter.Create(sPath, xmlWriterSettings);
		//XmlWriter xmlWriter = XmlWriter.Create(Application.dataPath +"/Data/resources.assets",xmlWriterSettings);
	...
	public void Save(int iSessionIndex, ref List<Molecule> ocean) {
		...
		// iOS can only write to its Documents directory
		#if UNITY_IPHONE
		sPath = Application.dataPath + "/../../Documents/psml";
		xmlWriter = XmlWriter.Create(sPath,xmlWriterSettings);
		#endif
	...
	private void saveNestedElements(XmlWriter xmlWriter, ref Session session, ref List<Molecule> ocean, string sParent)
	{
		...
		//int iType = PIXE.PSML_ELEMENT;
		if(API.hasElements (ref session, ref ocean, /*iType*/PIXE.PSML_ELEMENT)) {
			List<string> nestedElements = API.getElements (ref session, ref ocean, /*iType*/PIXE.PSML_ELEMENT);
			...
			foreach(string sElement in nestedElements) {
				if(sElement == sParent) {
					...
					//iType = PIXE.PSML_ATTRIBUTE;
					if(API.hasElements(ref session, ref ocean, /*iType*/ PIXE.PSML_ATTRIBUTE)) {	
						List<string>  attributes = API.getElements (ref session, ref ocean, /*iType*/PIXE.PSML_ATTRIBUTE);
						// Write each one
					...
					//iType = PIXE.PSML_ELEMENT;
					if(API.hasElements (ref session, ref ocean, /*iType*/PIXE.PSML_ELEMENT)) {
						List<string> nestedElements2 = API.getElements (ref session, ref ocean,/*iType*/ PIXE.PSML_ELEMENT);
	...
	void getJellyfish(XmlReader myReader, string elementName, int iSessionIndex) {
		...
		while (myReader.NodeType != XmlNodeType.EndElement) {
			...
			if(secondPass && !jellyfishWritten) {
				...
				// FOR TESTING ONLY
				API.Write(iSessionIndex,"Name","ALEXALEXALEX",ref iFlags);
				//
