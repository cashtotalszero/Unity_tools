...
public class MethodCalls : MonoBehaviour {
	...
	void Start() {
		...
		Write (thisSession, "psml://A/B/C/Catt1", 70, PIXE_PSML_WRITE_ATTRIBUTE);
		Write (thisSession, "psml://A/B/C/Catt2", 71, PIXE_PSML_WRITE_ATTRIBUTE);
		Write (thisSession, "psml://A/B/C/Catt3", 72, PIXE_PSML_WRITE_ATTRIBUTE);
		Write (thisSession, "psml://A/B/C/Catt4", 73, PIXE_PSML_WRITE_ATTRIBUTE);
		Write (thisSession, "psml://A/B/C/Catt5", 74, PIXE_PSML_WRITE_ATTRIBUTE);
		Write (thisSession, "psml://A/B/C/Catt6", 75, PIXE_PSML_WRITE_ATTRIBUTE);

		Write (thisSession, "psml://A/B/C/NEW", null, PIXE_PSML_WRITE_ELEMENT);
		Write (thisSession, "psml://A/B/C/NEW/Alex", 23, PIXE_PSML_WRITE_ATTRIBUTE);
		...
		Debug.Log ("Att that does exist outputs (AttB,65): " + ReadAt1);
		...
		ReadAt1 = Read (thisSession,"psml://A/B/D/Fox",PIXE_PSML_READ_ATTRIBUTE);
		Debug.Log ("Att that does exist outputs (Fox,29): " + ReadAt1);

		ReadAt1 = Read (thisSession,"psml://A/B/C/Catt6",PIXE_PSML_READ_ATTRIBUTE);
		Debug.Log ("Att that does exist outputs (Catt6,75): " + ReadAt1);

		ReadAt1 = Read (thisSession,"psml://A/B/C/NEW/Alex",PIXE_PSML_READ_ATTRIBUTE);
		Debug.Log ("Att that does exist outputs (Alex,23): " + ReadAt1);

		int x;
		string add;
		for (x=0; x<50; x++) {
			add = x.ToString();
			Write (thisSession, "psml://A/B/C/NEW/" +add, x, PIXE_PSML_WRITE_ATTRIBUTE);
		}

		Write (thisSession, "psml://A/B/C/NEW/Boom", null, PIXE_PSML_WRITE_ELEMENT);
		Write (thisSession, "psml://A/B/C/NEW/Boom/A", "X", PIXE_PSML_WRITE_ATTRIBUTE);


		long john = getDrop (31);
		Debug.Log ("???? = " + john);
	...
	private long getDropSize(long lMoleculeCount) {
		...
		if (lMoleculeCount >= 0 && lMoleculeCount <= 5) {
			lDropList = PIXE_OCEAN_DROP_5;
		}
		if (lMoleculeCount > 5 && lMoleculeCount <= 10) {
			lDropList = PIXE_OCEAN_DROP_10;
		}
		if (lMoleculeCount > 10 && lMoleculeCount <= 15) {
			lDropList = PIXE_OCEAN_DROP_15;
		}
		if (lMoleculeCount > 15 && lMoleculeCount <= 20) {
			lDropList = PIXE_OCEAN_DROP_20;
		}
		if (lMoleculeCount > 20 && lMoleculeCount <= 25) {
			lDropList = PIXE_OCEAN_DROP_25;
		}
		if (lMoleculeCount > 25 && lMoleculeCount <= 30) {
			lDropList = PIXE_OCEAN_DROP_30;
		}
		if (lMoleculeCount > 30) {
