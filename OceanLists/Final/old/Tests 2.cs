...
public class Tests : MonoBehaviour {
	...
	private void test6() {
		...
		/*int i,j; string toAdd,toAdd2,moved;
	
		for (i=0; i<100; i++) {
			toAdd = ("Jellyfish" + i);
			Jelly.Write (thisSession, toAdd, null, PIXE_PSML_WRITE_ELEMENT);
			Jelly.Move (thisSession, toAdd);
			for (j=0; j<7; j++) {
				toAdd2 = ("Att" + j);
				Jelly.Write (thisSession, toAdd2, j, PIXE_PSML_WRITE_ATTRIBUTE);
			}
			Jelly.Move (thisSession, "..");
		}
*/
		...
		Jelly.Write(thisSession,"Jellyfish1",null,PIXE_PSML_WRITE_ELEMENT);
		Jelly.Write(thisSession,"Jellyfish2",null,PIXE_PSML_WRITE_ELEMENT);
		Jelly.Write(thisSession,"Jellyfish3",null,PIXE_PSML_WRITE_ELEMENT);
		Jelly.Write(thisSession,"Jellyfish4",null,PIXE_PSML_WRITE_ELEMENT);
		Jelly.Write(thisSession,"Jellyfish5",null,PIXE_PSML_WRITE_ELEMENT);
		Jelly.Write(thisSession,"Jellyfish6",null,PIXE_PSML_WRITE_ELEMENT);


		int i,j; string toAdd,moved;
		for (j=1; j<6; j++) {
			for (i=0; i<50; i++) {
				toAdd = ("psml://Root/Jellyfish" + j + "/loop" + i);
				//moved = ("loop" + i);
				//Jelly.Move(thisSession, moved);
				Jelly.Write (thisSession, toAdd, i, PIXE_PSML_WRITE_ATTRIBUTE);
			}
		}
		...
		for (j=1; j<6; j++) {
			for (i=0; i<6; i++) {
				nested = ("Nested"+i);
				zulu=("zulu"+i);
				toAdd = ("psml://Root/Jellyfish" + j +"/"+nested);
				Jelly.Write(thisSession,toAdd,null,PIXE_PSML_WRITE_ELEMENT);
				Jelly.Move(thisSession, nested);
				Jelly.Write (thisSession,zulu, i, PIXE_PSML_WRITE_ATTRIBUTE);
			}
		}*/
