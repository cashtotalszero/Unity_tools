...
public class Heap : MonoBehaviour {
		...
		if (iFlags >= PIXE_OP_SUCCESSFUL) {
			...
			if (iFlags == PIXE_PSML_WRITE_ELEMENT) {
				...
				createMolecule (
					ref session, ref ocean,
					(iCursor + iDropSize), iCursor, 
					sPath, "A", oValue, (oValue.GetType ()).ToString ());
			}
	...
	private void createMolecule(ref Session session, ref List<Molecule> ocean, 
	                            ...
	                            string sType, object oValue, object oData)
	{
		createmolecule++;
	...
	private void createNested(ref Session session, ref List<Molecule> ocean, int iParentHeader)
	{
		...
		createMolecule(
			ref session, ref ocean,
			iCursor, iCursor, 
			mHeader.Name, "H", 0, (iParentHeader - iCursor));   /// CHANGE 1 to 0 if wrong
	...
	// THERE IS AN ISSUE HERE WITH TEST 6
	...
	private int getDrop(ref Session session, ref List<Molecule> ocean, int iSize, int iHeader, bool bMove) 
	{
		...
		while (!bDone) {
			...
			if (iLast >= 0) {
				...
				if(iFoundDrop==391) {
					Debug.Log("ISSUE FOUND IN "+iDropList);
				}
				...
				//Debug.Log("Found drop = "+iFoundDrop+ " vs Last molecule = "+iLastMolecule);
				//Debug.Log("New drop end = "+iNewDropEnd);

				if(iNewDropEnd >= iLastMolecule) {
					int i;
					for(i=iLastMolecule;i<(iNewDropEnd/*+iSize*/);i++) {
	...
	private void moveDrop(ref Session session, ref List<Molecule> ocean, ref int iHeader) 
	{
		...
		if (ocean [iNewLocation].Name != "" && ocean [iNewLocation].Name != null) {
			Debug.Log("OVERLAP !!!!!!!!!! "+iNewLocation);
		...
		List<int>[] dropArray = (List<int>[])ocean[PIXE_OCEAN_HOME].Data;
		dropArray [iDropList].Add (iNewFree);
		*/
